...
model_name = "Model_L3_%s_for_%s_nbasistime%d_nbasisloc%d_seqs%d" % (
    loss_type, data_name, n_basis_time, n_basis_loc, use_seq
)
run_tag = time.strftime("%Y%m%d-%H%M%S")   # e.g., 20251226-213522
model_name = f"{model_name}_{run_tag}"
save_path = f"results/saved_models/{model_name}"
G = torch.load(f"data/{data_name}/graph.pt", weights_only=False)
